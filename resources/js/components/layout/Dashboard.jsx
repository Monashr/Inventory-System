    ...
    <SidebarProvider>
      <Sidebar variant="inset" className="border-r border-border/40">
                    ...
                    <Collapsible key={idx} defaultOpen={isParentActive}>
                      <SidebarMenuItem>
                        <CollapsibleTrigger asChild>
                          <SidebarMenuButton
                            className={cn("h-9 px-3 rounded-lg transition-all duration-200 hover:bg-accent/50")}
                          >
                        ...
                        <CollapsibleContent>
                          <SidebarMenuSub className="ml-4 mt-1 border-l border-border/40 pl-4">
                            {menu.children.map((child, cidx) => {
                              const isActive = currentPath.startsWith(child.route)
                              return (
                ...
                return (
                  <SidebarMenuItem key={idx}>
                    <SidebarMenuButton
                      ...
                      className="h-9 px-3 rounded-lg transition-all duration-200 hover:bg-accent/50"
                    >
                      <Link href={menu.route} className="flex items-center gap-3">
                        <Icon className="h-4 w-4" />
                        <span className="font-medium">{menu.title}</span>
                      </Link>
                    </SidebarMenuButton>
        ...
        <SidebarFooter className="border-t border-border/40 p-2">
          <SidebarMenu className="space-y-1">
            <SidebarMenuItem>
              <SidebarMenuButton asChild size="sm" className="h-8 px-3 rounded-lg">
            ...
            <SidebarMenuItem>
              <SidebarMenuButton asChild size="sm" className="h-8 px-3 rounded-lg">
                <Link href="/dashboard/tenant" className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className="h-10 px-3 rounded-lg hover:bg-accent/50 transition-colors duration-200"
                    >
                      <div className="flex items-center gap-3">
                        <Avatar className="h-7 w-7 rounded-lg">
                          <AvatarImage src={`/storage/${user.picture}`} alt={user.name} />
