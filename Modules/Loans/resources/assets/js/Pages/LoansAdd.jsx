...
function LoansAdd() {
  ...
  const handleAddToCart = (asset) => {
    if (!cart.find((item) => item.id === asset.id)) {
      setCart([...cart, { ...asset, date: new Date().toISOString().split("T")[0] }])
  ...
  const handleDateChange = (id, date) => {
    setCart(cart.map((item) => (item.id === id ? { ...item, date } : item)))
  }
  ...
  const getConditionConfig = (condition) => {
    ...
    return configs[condition] || "bg-gray-100 text-gray-800 border-gray-200"
  }

  const canSubmit = selectedUser && cart.length > 0 && cart.every((item) => item.date)
        ...
        <CardContent className="p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Create New Loan</h1>
                <p className="text-sm text-gray-600 dark:text-gray-400">Select assets and assign to an employee</p>
              </div>
            </div>
              ...
              {filteredAssets.length > 0 ? (
                    ...
                    return (
                      <Card
                        ...
                        className={`transition-all duration-200 hover:shadow-md ${
                          isInCart ? "ring-2 ring-green-500 bg-green-50 dark:bg-green-950" : "hover:shadow-lg"
                        }`}
                      >
                        <CardContent className="p-4 space-y-3">
                          <div className="space-y-1">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400 font-mono">{asset.serial_code}</p>
                          </div>

                          <div className="flex items-center justify-between">
                            <span className="text-xs text-gray-500">Condition:</span>
                            <Badge variant="outline" className={`text-xs ${getConditionConfig(asset.condition)}`}>
                          ...
                          {isInCart ? (
                            ...
                            <Button size="sm" className="w-full" onClick={() => handleAddToCart(asset)}>
                ...
                <div className="flex flex-col items-center justify-center h-64 space-y-4">
                  ...
                  <div className="text-center space-y-2">
                    <p className="text-lg font-medium text-gray-700 dark:text-gray-300">No Assets Found</p>
              ...
              {selectedUserData && (
                <div className="p-2 bg-blue-50 dark:bg-blue-950 rounded-md">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    Selected: <span className="font-medium">{selectedUserData.name}</span>
                  </p>
                </div>
              )}
            </div>
                ...
                <ScrollArea className="h-full p-2">
                  {cart.length > 0 ? (
                          ...
                          <CardContent className="p-3 space-y-3">
                            <div className="flex justify-between items-start">
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-600 dark:text-gray-400 font-mono">{item.serial_code}</p>
                              </div>
                              <Button
                                variant="ghost"
                                size="sm"
                            ...
                            <div className="space-y-1">
                              <Label className="text-xs text-gray-500">Loan Date</Label>
                              <Input
                                type="date"
                                value={item.date}
                    ...
                    <div className="flex flex-col items-center justify-center h-32 text-center space-y-2">
                      ...
                      <p className="text-xs text-gray-400">Add assets from the left panel</p>
                    </div>
                  )}
                </ScrollArea>
              </div>
              ...
              <div className="space-y-2 shrink-0">
                {!selectedUser && (
                  <Alert>
                    ...
                    <AlertDescription className="text-sm">Please select an employee</AlertDescription>
                  </Alert>
                )}
                ...
                {cart.some((item) => !item.date) && (
                  <Alert>
                    ...
                    <AlertDescription className="text-sm">Please set loan dates for all assets</AlertDescription>
                  </Alert>
                )}
              </div>
            )}
            ...
            <div className="pt-4 border-t shrink-0">
              <Button className="w-full" disabled={!canSubmit || isSubmitting} onClick={handleSubmit}>
                {isSubmitting ? (
                  ...
                  <>
                    <CheckCircle2 className="w-4 h-4 mr-1" />
                    Create Loan ({cart.length} {cart.length === 1 ? "asset" : "assets"})
                  </>
                )}
              </Button>

              {cart.length > 0 && (
                <p className="text-xs text-gray-500 text-center mt-2">Review all details before creating the loan</p>
              )}
            </div>
          </CardContent>
        </Card>
